...
import recordItem from "./recordsItem";
...
export default class NewClass extends cc.Component {
    ...
    public iconPf:cc.Prefab = null
     ...
     /**加载预制体 */
     async loadCard(){
        this.iconPf =  await loaderManager.getRes("recordsItem","prefab",cc.Prefab)
        if(this.iconPf){
    ...
    instantNode(index:number){
        let itemNode:cc.Node = cc.instantiate(this.iconPf)
        this.chartsLayout.addChild(itemNode)
        itemNode.setPosition(0,0)
        itemNode.getComponent(recordItem).init(index)
    }
