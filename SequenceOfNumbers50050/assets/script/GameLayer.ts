...
import { DirectionType, EventId, JsbFunType } from './util/define';
...
export default class GameLayer extends cc.Component {
    ...
    btnEvent(event: cc.Event) {
        ...
        switch (name) {
            ...
            case 'tipBtn':
                ...
                break;
            case 'btnAds':
                DataManager.nativeJsbFun(JsbFunType.ShowRewardedVideoAd);
                break;
            case 'closeTipBtn':
                this.hideTipNode();
                break;
    ...
    /**创建文字 */
    creatLaber(index: number, val: number) {
        let labeNode = cc.instantiate(this.iconPf);
        labeNode.setParent(this.gamePos);
        labeNode.getComponent(ItemNode).init(val, index);
