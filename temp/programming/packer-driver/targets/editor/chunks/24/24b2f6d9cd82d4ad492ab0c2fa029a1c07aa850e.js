  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", ResMgr = class ResMgr extends (_crd && Singleton === void 0 ? (_reportPossibleCrUseOfSingleton({
        ...
        getRes(path, typeRes) {
          let resName = this.getResourcesName(path); //如果名称为name的节点池不存在，就新建一个；如果存在，判断大小，有就取出来用
        ...
        getResPool(name, typeRes) {
          ...
          let resPool = this._allResources[typeName];
        ...
        getNodePool(name) {
          let resPool = this._allResources.NodePool[name];
...
//# sourceMappingURL=24b2f6d9cd82d4ad492ab0c2fa029a1c07aa850e.js.map