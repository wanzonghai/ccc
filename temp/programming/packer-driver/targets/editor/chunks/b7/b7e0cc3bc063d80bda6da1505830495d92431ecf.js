    ...
    execute: function () {
      ...
      _export("Until", Until = class Until {
        ...
        getNumberToString(value) {
          let str = value.toString(); //大于16位，用e表示的大数字，需要转换

          if (str.indexOf('e') != -1) {
            //e位置
            let eIndex = str.indexOf('e'); //小数点位置

            let pIndex = str.indexOf('.'); //是否有小数点, 400000000...这样的数是没有小数点的

            let bHavePoint = pIndex; //小数点位置

            pIndex = pIndex == -1 ? 0 : pIndex; //小数点后的数字位数

            let small = eIndex - pIndex - 1; //e前面的数字

            let result = str.substr(0, eIndex); //如果e前面数字有小数点，则去掉小数点
            ...
            let eNum = str.substr(eIndex + 2, str.length); //需要添加的0数量
        ...
        static formatNumber3(numStr) {
          ...
          let lenStr = numStr.length;
          let newNumStr = '';
          let blws = lenStr > 6 ? 5 : 6;
          ...
          let num0 = parseInt(newNumStr);
          /**发出的数据 */

          let zsnum = num0; // //获取最高位值
          // num1 = num / mVlue;
          ...
          if (lenStr < 3) {
            ...
            let numLen_1 = lenStr - 8;
            let numm = 0;
            let numLen_2 = numLen_1 - 9;
            let num2 = Math.floor(numLen_1 % 3);
            if (lenStr <= 11) zmfh = String.fromCharCode(109); //m
              ...
              let aanum = Math.floor(numLen_2 / 3);
              let aAd = num2 == 0 ? 0 : 1;
        ...
        static tweenFloat(from, to, duration, onUpdate, onComplete, autoStart = true) {
          ...
          let _tween = tween(o).to(duration, {
            value: to
          }).call(onComplete);
...
//# sourceMappingURL=b7e0cc3bc063d80bda6da1505830495d92431ecf.js.map