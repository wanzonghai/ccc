import { AssetManager, SpriteFrame, VERSION, assetManager } from "cc";
...
class loaderManager{
    ...
    */
     ...
     * 加载资源
    */
    public async loadRes(fileName, targetBundleNmae?, type = SpriteFrame) {
        let res = null;
        let loadBundle = (bundleName) => {
            return new Promise<void>((resolve, reject) => {
                console.log("当前加载包名：", bundleName);
                assetManager.loadBundle(bundleName,(err, ab: AssetManager.Bundle)=> {
                    if(err) {
                    ...
                    if(type == SpriteFrame && VERSION[0] == "3") { // 要加载纹理需要获取图片下的spiteFrame
