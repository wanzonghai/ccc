...
export default class BallNode extends cc.Component {
    ...
    init(type: ItemType) {
        ...
        cc.tween(this.node)
            ...
            .then(cc.tween(this.node).repeatForever(cc.tween().to(0.5, { scale: 1.1 }).to(0.5, { scale: 1 })))
            .start();
    }

    moving() {
        cc.tween(this.node).repeatForever(cc.tween().to(0.5, { scale: 1.1 }).to(0.5, { scale: 1 }));
        cc.tween(this.node)
            .to(0.2, { scale: 1.2 })
            .to(0.2, { scale: 1 })
