...
export default class BallNode extends cc.Component {
    ...
    protected onEnable(): void {
        cc.tween(this.node)
            .to(0.2, { scale: 1.4, opacity: 255 }) // 增加缩放幅度和完全可见的透明度
            .to(0.2, { scale: 1, opacity: 128 }) // 恢复正常缩放并降低透明度
            .call(() => {
                this.moving();
            })
            .then(
                cc.tween(this.node).repeatForever(
                    cc
                        .tween()
                        .to(0.5, { scale: 1.2, opacity: 255 }) // 增加缩放和透明度
                        .to(0.5, { scale: 1, opacity: 128 }) // 恢复正常缩放和降低透明度
                )
            )
            .start();
    }
