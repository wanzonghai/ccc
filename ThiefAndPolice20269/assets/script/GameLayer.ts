...
export default class GameLayer extends cc.Component {
    ...
    */
    ...
    movingBone(taget:cc.Node,direction:DirectionType){
        let movingPos:cc.Vec3
        let speed:number = 150
        let dir:number = 1
        switch(direction){
            case DirectionType.Top:
                // console.log("上");
                movingPos = cc.v3(taget.position.x,taget.position.y + speed,0)
                if(taget.position.y + speed > 650){
                    movingPos = cc.v3(taget.position.x,700,0)
                }
                dir = taget.scaleX
            break;
            case DirectionType.Bottom:
                // console.log("下");
                movingPos = cc.v3(taget.position.x,taget.position.y - speed,0)
                if(taget.position.y - speed < -530){
                    movingPos = cc.v3(taget.position.x,-630,0)
                }
                dir = taget.scaleX
            break;
            case DirectionType.Left:
                movingPos = cc.v3(taget.position.x - speed,taget.position.y,0)
                dir = 1
                if(taget.position.x - speed < -436){
                    movingPos = cc.v3(-436,taget.position.y,0)
                }
                // console.log("左");
            break;
        ...
        .to(0.05,{scaleX:dir})
        .call(()=>{
            taget.getComponent(cc.Animation).play('thift')
        })
        .to(0.3,{position:movingPos})
        .start()
     ...
     /**绑定滑动 */
     listThief(){
         this.ThiefNode.on(cc.Node.EventType.TOUCH_START,(event)=>{
             this.ThiefNode["startPos"] = cc.v2(event.getLocation().x, event.getLocation().y)
          })
          this.ThiefNode.on(cc.Node.EventType.TOUCH_CANCEL,(event)=>{
             let statPos = this.ThiefNode["startPos"]
             ...
             if(direction){
                 this.movingBone(this.ThiefNode,direction)
             }
          })
     }
     ...
     /**取消绑定滑动 */
     offBallEvent(){
         this.ThiefNode.off(cc.Node.EventType.TOUCH_START)
         this.ThiefNode.off(cc.Node.EventType.TOUCH_CANCEL)
     }
