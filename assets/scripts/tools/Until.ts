import { tween, Node, Sprite, SpriteFrame, resources, Label, Texture2D, Vec2, assetManager, v2, UITransform } from 'cc';
...
export class Until {
     ...
     * 重复自转
     */
    public static actionSystem(targetNode: Node, type: number): void {
        switch (type) {
            case 1: //重复自转
                tween(targetNode).by(5, { angle: 360 }).repeatForever().start();
                break;
            case 2: //呼吸
                tween(targetNode)
                    .repeatForever(tween().by(3, { scale: -0.3 }).by(1, { scale: 0.3 }))
                    .start();
    ...
    /**2020-10-10 */
    public static formatTimeData(num: number, type: number): string {
    ...
    /** 返回格式为xx天xx小时xx分钟 */
    public static formatTimeDay(autoTime1: number, autoTime2: number) {
    ...
    /** 返回秒 */
    public static formatTimeSec(autoTime1: number, autoTime2: number) {
        var date2 = (autoTime1 + autoTime2) * 1000;
        var date3 = Date.now(); //获取当前标准时间
        var date4 = date2 - date3; //时间差的毫秒数

        //计算出相差天数
        var secs = Math.floor(date4 / 1000);
        // //计算出小时数
        // var leave1 = date3 % (24 * 3600 * 1000); //计算天数后剩余的毫秒数
        // var hours = Math.floor(leave1 / (3600 * 1000));
        // //计算相差分钟数
    ...
    /**数据转换k m b t a  */
    public static formatNumber3(numStr: string): string {
        // let zm = {
        //     k: 'k',
        //     m: 'm',
        //     b: 'b',
        ...
        let zmfh: string = '';
        let ASCIICode: number = 97;
        /**最高位 */
        let lenStr: number = numStr.length;
        let newNumStr: string = '';
        let blws: number = lenStr > 6 ? 5 : 6;
        for (let index = 0; index < lenStr; index++) {
            const element = numStr[index];
            if (index >= blws) break;
            newNumStr += element;
        }
        let num0: number = parseInt(newNumStr);

        /**发出的数据 */
        let zsnum: number = num0;
        // //获取最高位值
        // num1 = num / mVlue;
        /**输出值2 */
        ...
        if (lenStr < 3) {
            ...
            let numLen_1: number = lenStr - 8;
            let numm: number = 0;
            let numLen_2: number = numLen_1 - 9;
            let num2: number = Math.floor(numLen_1 % 3);
            if (lenStr <= 11) zmfh = String.fromCharCode(109); //m
                ...
                let aanum: number = Math.floor(numLen_2 / 3);
                let aAd: number = num2 == 0 ? 0 : 1;
                if (aanum > 0) {
                    ASCIICode += aanum;
    ...
    public static tweenFloat(from: number, to: number, duration: number, onUpdate: (t: number) => void, onComplete?: Function, autoStart: boolean = true) {
        ...
        let _tween = tween(o).to(duration, { value: to }).call(onComplete);
        if (autoStart) {
            _tween.start();
        }
        return _tween;
    ...
    public static AddHeadSprite(url: string, Sprite_head: Sprite) {
        assetManager.loadRemote(url, { ext: '.png' }, function (err, tex) {
    ...
    public static generateRandomName(): string {
        ...
        const nouns = ['Player', 'Gamer', 'Hero', 'Ninja', 'Wizard', 'Pirate'];
    ...
    // 随机生成玩家ID
    public static generateRandomPlayerId(): string {
        return Math.random().toString(36).substring(2, 12);
    }

    // 随机生成玩家avatar
    public static generateRandomAvatar(): string {
